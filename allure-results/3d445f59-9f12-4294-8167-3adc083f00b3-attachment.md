...
> 80 |         expect(await qaActor.asks(TheLastResponse.status())).toBe(201);
     |                                                              ^ Error: expect(received).toBe(expected) // Object.is equality
  81 |       });
  82 |   test('API returns correct content-type header', async ({ qaActor }) => {
  83 |     await qaActor.attemptsTo(ManageUsers.getPage(1));
  84 |     const contentType = await qaActor.asks(TheLastResponse.header('content-type'));
  85 |     expect(contentType).toContain('application/json');
  86 |   });
  87 | });
  88 | 
```