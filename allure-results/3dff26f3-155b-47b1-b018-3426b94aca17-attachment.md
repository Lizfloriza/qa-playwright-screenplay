...
- Name: api/auth.spec.ts >> Auth API >> POST /api/login >> should login with valid credentials and return token
- Location: tests/api/auth.spec.ts:16:9

# Error details

```
Error: Path "token" not found in response body
```
