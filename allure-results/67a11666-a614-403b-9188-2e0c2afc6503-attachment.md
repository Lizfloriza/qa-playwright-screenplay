...
> 78 |     expect(await qaActor.asks(TheLastResponse.status())).toBe(200);
     |                                                          ^ Error: expect(received).toBe(expected) // Object.is equality
  79 |   });
  80 |   test('API returns correct content-type header', async ({ qaActor }) => {
  81 |     await qaActor.attemptsTo(ManageUsers.getPage(1));
  82 |     const contentType = await qaActor.asks(TheLastResponse.header('content-type'));
  83 |     expect(contentType).toContain('application/json');
  84 |   });
  85 | });
  86 | 
```