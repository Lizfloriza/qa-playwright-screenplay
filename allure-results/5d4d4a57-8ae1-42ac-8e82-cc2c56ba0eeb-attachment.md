...
- Name: regression/critical-path.spec.ts >> Critical Path — User Lifecycle >> should create a put and return 200
- Location: tests/regression/critical-path.spec.ts:76:7
...
```ts
  ...
  75 |   });
  76 |   test('should create a put and return 200', async ({ qaActor }) => {
  77 |     await qaActor.attemptsTo( Send.PUT().to('/posts/1').withBody({ title: 'Test Put', body: 'Content', userId: 1 }), );
> 78 |     expect(await qaActor.asks(TheLastResponse.status())).toBe(200); });
     |                                                          ^ Error: expect(received).toBe(expected) // Object.is equality
  79 |   test('API returns correct content-type header', async ({ qaActor }) => {
  80 |     await qaActor.attemptsTo(ManageUsers.getPage(1));
  81 |     const contentType = await qaActor.asks(TheLastResponse.header('content-type'));
  82 |     expect(contentType).toContain('application/json');
  83 |   });
  84 | });
  85 | 
```