...
- Name: api/auth.spec.ts >> Auth API >> POST /api/register >> should register successfully and return id + token
- Location: tests/api/auth.spec.ts:53:9

# Error details

```
Error: Path "id" not found in response body
```
